...
#    The supervised launch is driven by `ai-env run`, which wires the
#    supervisor, control socket, provider proxy, MCP gateway, optional
#    shell shim, and egress observer around the chosen agent launcher.
ai-env run demo --agent claude --task "fix the failing tests"
# Optional flags: --continue (resume from the env's most recent run),
# --shell-shim (wrap in-sandbox shell calls), --observer-mode strict.
...
- `--run <id>`: optional. Pin the scan artifacts to a specific historical run by its directory basename. Defaults to the env's latest run. The command requires at least one run to exist for the env (typically produced by a prior `ai-env run`).
...
Reclaims an env's on-disk resources. The command is the inverse of `ai-env new`: it walks upward from the current directory to find the project's `.ai-env/` (same lookup as `list` / `diff` / `patch`), reads the env's `.env-meta.json` to learn the strategy, then removes:

- The workspace directory at `.ai-env/workspaces/<env-name>/` (every strategy).
- The read-only baseline snapshot at `.ai-env/baselines/<env-name>/` (copy strategy only).
- The per-env branch `ai-env/<env-name>` from the source repository (worktree strategy only). The worktree itself is removed via `git worktree remove --force` so an env left in a dirty state on disk still gets reclaimed.

Run records under `.ai-env/runs/<env-name>/` and the project's shared configuration files (`ai-env.yaml`, `policy.yaml`, `agents.yaml`, secrets files) are intentionally left in place: an operator who destroys an env to start over keeps their policy and the audit trail of past runs.

Destroy is idempotent. Running `ai-env destroy <env>` twice in a row exits 0 both times: the first invocation removes the workspace, baseline, and branch (whichever apply); the second sees no resources to reclaim and prints an "already absent" notice on stderr without failing, so the command is safe to drop into cleanup scripts. The summary printed to stdout uses stable labels (`Destroyed env "<name>"`, then a per-line `strategy:`, `workspace:`, `baseline:`, `branch:` breakdown) so scripts and acceptance tests can grep the verdict.
...
Runs the host-side diagnostic the supervisor walks before a run. Where `ai-env agents doctor` reports on per-agent CLIs, `ai-env doctor` reports on the host surface every run depends on: platform identification, the `internal/capability` snapshot (CAP_NET_ADMIN, CAP_SYS_ADMIN, NFLOG / pflog reach, slirp4netns, bridge gateway, `/dev/fd`, userns-remap), container-runtime probes (`docker` and `podman` with a short five-second probe timeout so a wedged daemon socket cannot hang the diagnostic), and optional security tooling (`git`, `gitleaks`). Each row renders as `PASS`, `WARN`, or `FAIL` with a short reason string; only a missing capability or dependency that would block a run at all on this host is rendered as `FAIL`. Surfaces the supervisor degrades around (no NFLOG yields observer unavailable, no `/dev/fd` yields no single-fd exec) render as `WARN`.

After the checks, `doctor` prints a remediation table mapping every lifecycle verb the supervisor can emit (`network_policy_degraded`, `shim_coverage_degraded`, `helper_rpc_aborted`, `transcript_parser_error`, `mcp_config_neutralized`, `gateway_secret_blocked`, `gateway_secret_response`, `origin_drift`, etc.) to the docs page or config knob an operator should consult when it fires. The JSON form (`--json`) emits the same rows in a stable schema so a CI job can pin one field without the table format breaking it. See `docs/operator-runbook.md` for the troubleshooting playbook the verbs reference and `docs/platform-parity.md` for the per-platform feature matrix.
...
Note: the supervisor primitives, status, logs, list-with-run-state, and `--continue` plumbing all landed in plan 03. The Backend interface, agent launchers, and `ai-env agents` subcommands documented below landed in plan 04 alongside the supervisor's optional `BackendAdapter` seam. Plan 05 added the canonical `NetworkPolicy`, the `NetworkPolicyAdapter` interface, the docker_sbx network adapter, the rootless docker / podman fallback backends, the host-side provider proxy, the `network-events.jsonl` writer, the `ai-env report` subcommand, and the `final-summary.md` writer (with the network section). Plan 06 added the built-in pattern-only secret scanner, the warn-only entropy analyzer, the gitleaks adapter, optional external scanner discovery, the `ai-env scan` subcommand, the `ExportGate` (wired into `ai-env patch` and `ai-env pr`), and the `ScanHook` seam the supervisor invokes during `StateScanning`. Plan 07 added the GitHub broker (`internal/githubbroker/`): the `GitHubBroker` interface, the GitHub App installation token and PAT-fallback `TokenSource` implementations, the in-memory `TokenHolder` with TTL clamping and revocation, branch-prefix / protected-branch / path-gate validation, PR-metadata secret scanning, broker-generated PR body, log redaction (`RedactTokens`, `NewRedactingLogger`, `NewRedactingWriter`), the `ai-env pr` broker wiring with the `ExportGate` running first, and the `PolicyDecisionsWriter` that persists gate verdicts and broker lifecycle outcomes into `policy-decisions.jsonl`. Plan 08 added the `PolicyEngine` package with allow / deny / ask decisions, the `ai-env policy` subcommands (`init`, `check`, `explain`, `allow|deny domain|tool`), the experimental `--shell-shim` prototype and supervisor wiring (`SupervisorOptions.ShellShim` / `ShellShimDir`), the engine-decisions extension to `policy-decisions.jsonl`, CI workflows for the race detector / integration tests / release dry-run, the operator-facing docs under `docs/` (`threat-model.md`, `enforcement-boundaries.md`, `residual-risk.md`, `model-credentials.md`, `backends.md`, `unsafe-modes.md`), minimal Node and Python fixtures under `tests/fixtures/`, the MVP-demonstration acceptance tests under `tests/acceptance/`, and the `ai-env destroy` subcommand (workspace + baseline + worktree-branch reclamation, idempotent on the re-run path). Plan 09 added the MCP gateway subsystem under `internal/mcp/`: the `MCPRegistry` loader and validator with deny-by-default semantics and strict YAML decoding, source / digest / schema-hash pinning, the gateway proxy (`AuthorizeLaunch` and `AuthorizeCall` returning deterministic `block` / `warn` / `allow` / `record` outcomes), the workspace-only filesystem scope enforcer (`scope_filesystem.go`) and current-repo-only GitHub scope enforcer (`scope_github.go`), the `ai-env mcp list / add / pin / scan / remove` subcommands wired in `internal/cli/mcp.go`, the per-run `mcp-calls.jsonl` writer in `internal/run/mcp_calls.go` (bridged from the gateway via `internal/cli/mcp_logger.go`), the operator-facing reference at `docs/mcp-security.md`, and the end-to-end acceptance tests under `tests/acceptance/mcp_gateway_test.go` covering unknown-server block, schema-hash drift, filesystem-scope escape, and GitHub cross-repo refusal. Plan 10 added the leak-coverage hardening pass: the per-run JSON-RPC control socket (`internal/run/control_socket.go`) with a primary control token plus per-MCP-server tokens and graceful shutdown, the canonical lifecycle-verb constants and per-verb metadata (`internal/run/lifecycle_verbs.go`) covering proxy, gateway, observer, broker, control-socket, policy-degradation, shim-coverage, helper, transcript, and MCP-config events, the unified per-run `leaks.jsonl` writer (atomic "write tmp, fsync, rename" at finalize time) plus the cross-stream aggregator in `internal/run/leaks_aggregate.go`, the hidden `ai-env shim-helper {shell,mcp}` subcommand that exec's via `SYS_EXECVEAT` on Linux or `/dev/fd/<n>` on macOS for TOCTOU-safe scan-and-exec and runs a JSON-aware stdio MCP shim with a streaming secret scanner, the canonical shim shadow set in `internal/policy/shim_install.go` and `internal/policy/shim_programs.go`, the capability snapshot in `internal/capability/` (CAP_NET_ADMIN, setns, userns-remap, pflog), the backend mount split (`BindMount`, `EnvSpec.BindMounts`, `EnvSpec.UID`, `EnvSpec.HomeTarget`, `BackendEventSink`) that separates the host-only `<runDir>/` from the agent-visible `<runDir>/ipc/`, the `.ai-env/secrets.local.yaml` loader with the mode-0600 enforcement check (`internal/secrets/local.go`), the multi-provider ProviderProxy with per-provider Host allowlist (`internal/secrets/proxy.go`) and reachability mode selection (`SetnsTCP`, `BridgeGateway`, `UnixSocket`), `BuildProviderProxyFromSecrets` and `BuildBrokerFromSecrets` factories, the BuildRunGateway factory for the per-run MCP gateway with per-server short-lived tokens, the workspace-MCP-config shadowing at `backend.Create` (`*.ai-env-shadowed`), the per-direction streaming JSON-aware MCP secret detector, the GitHub broker origin pin captured at `ai-env new` and re-checked at PR time (refused on `origin_drift`), the Linux NFLOG observer (`internal/egress/nflog/`) and macOS pflog observer (`internal/egress/pflog/`) plus the per-run `iptables` / `pf` rule lifecycle (`internal/egress/rules/`), `network_policy_degraded` emission via `BackendEventSink`, the `filesystem-events.jsonl` writer (emitted by the MCP gateway and the shell shim), the `transcript.jsonl` writer with per-CLI parsers for Claude Code stream-json and Codex `--json` plus turn-id correlation through the control socket, the `ai-env leaks` reader subcommand with `--format`, `--vector`, `--source`, and `--run` filters, the `ai-env doctor` host-side diagnostic subcommand (capability snapshot, runtime probes, remediation table per lifecycle verb), the canonical 11-step pre-launch and 9-step teardown sequence in the supervisor (`supervisor_sequence.go`), the run directory `chmod 0700` and the stale-tmp sweeper at supervisor start, and the schema reference docs (`docs/leaks-jsonl-schema.md`, `docs/transcript-jsonl-schema.md`, `docs/filesystem-events-jsonl-schema.md`), platform-parity matrix (`docs/platform-parity.md`), operator runbook (`docs/operator-runbook.md`), and `secrets.local.yaml` reference (`docs/secrets-local-yaml.md`). The user-facing `ai-env run` subcommand that wires the supervisor, the backend, the network adapter, the provider proxy, and the scan hook together end-to-end ships in `internal/cli/run.go` (registered via `cmd/ai-env/main.go::newRunCmd`). Flags are `--task` (required), `--agent` (defaults to `project.default_agent` in `ai-env.yaml`), `--continue`, `--shell-shim`, and `--observer-mode {auto|strict|disabled}`.
...
- `ShellShimDir string`: absolute path of the directory holding the wrapper binary. The supervisor does not invent the directory: the caller (`ai-env run` or a test fixture) materializes the wrapper there before the run starts.

`run.InjectShimPath(env, shimDir)` is the exported helper the supervisor (and `ai-env run`) use to mutate the PATH entry: it prepends `shimDir` to an existing `PATH=` entry or appends a fresh `PATH=<shimDir>` when the env did not carry one.
...
The user-facing CLI flag `ai-env run --shell-shim` is shipped (see `cmd/ai-env/main.go::newRunCmd`); the `SupervisorOptions.ShellShim` field and the shell-shim wiring are the underlying primitives that flag toggles.
...
The MCP gateway in `internal/mcp/` is the v0.2 chokepoint that sits between the agent and any Model Context Protocol server the operator has registered. The gateway runs on the host: it loads the registry, compares the server's advertised tool list against the pinned schema hash at launch, dispatches every per-call request through a scope enforcer, and appends each `AuthorizeLaunch` and `AuthorizeCall` verdict to the per-run audit log. The cardinal rule is that the agent's MCP client talks only to the gateway proxy; bypassing it bypasses the whole MCP security model. The operator-facing reference is `docs/mcp-security.md`, which covers the threat model, schema-hash workflow, scope semantics, audit format, CLI surface, and the operator playbook.

The pieces:

- **Registry (`internal/mcp/registry.go`).** Loads `.ai-env/mcp.yaml` with strict (`KnownFields(true)`) decoding. `version` must be `1`, `default` must be `deny`, and the `servers` map must be non-empty; an unknown server name returns `ErrUnknownServer` so the gateway translates it into a `block` outcome with reason `"unknown server"`. Each entry pins a `source` (`npm:<pkg>@<version>` or `oci:<image>:<tag>`), optionally a content `digest` (`sha256:<hex>`), optionally a `schema_hash` (`sha256:<hex>`), one or more `scope` blocks (`filesystem.root: workspace_only` and / or `github.repos: current_repo_only` plus `github.operations: read_only | read_write`), and a per-server `policy` token (`allow`, `deny`, or `warn`). `MatchVersion` and `MatchDigest` are exact string equality; any drift between `mcp.yaml` and what the launcher resolved is a block, never a warning.
- **Schema-hash pin (`internal/mcp/schema.go`).** `ComputeSchemaHash` canonicalizes the server's advertised tool list (sorted by name, each tool serialized as name + description + canonical JSON schema) and SHA-256s the result. The gateway compares the live hash against the registered hash at launch: equal hashes pass; a mismatch routes through the per-server policy (`block`, `warn`, or `record` depending on configuration) so an upstream tool-description change cannot silently broaden the agent's effective capabilities.
- **Gateway proxy (`internal/mcp/gateway.go`).** `Gateway.AuthorizeLaunch` is called once per server when the agent connects: it resolves the registry entry, runs the version / digest / schema-hash checks, and returns a `GatewayOutcome` (`block`, `warn`, `allow`, or `record`) plus a reason string. `Gateway.AuthorizeCall` runs on every tool invocation: it dispatches to the per-scope enforcer, applies the per-server policy, and returns the same outcome shape. The outcomes are deterministic and side-effect free aside from the audit log write, so the gateway can be unit-tested against fixture registries without touching the network.
- **Scope enforcers (`scope_filesystem.go` and `scope_github.go`).** The filesystem enforcer captures the env's workspace root on the host at construction; any path argument that does not resolve inside `.ai-env/workspaces/<env-name>/` after symlink expansion is a block. The GitHub enforcer captures the current repo coordinate (owner + name) at construction; any tool argument that names a different repo is a block, and any operation outside the registered `github.operations` cap (e.g. a `write_*` call when `read_only` is registered) is also a block. Both enforcers are pure functions over the registry view and the per-call arguments; the runtime context (workspace root, current repo) is host-supplied and not reachable from inside the sandbox.
- **Per-server policy.** The registered `policy` token (`allow` / `deny` / `warn`) is the operator's final say. `deny` short-circuits the gateway with a block, `warn` records a warning but lets the call through, and `allow` defers to the scope enforcers. A mismatched schema hash respects the per-server policy too, so an operator who wants "warn on schema drift but still record" can do that without code changes.
- **CLI surface (`internal/cli/mcp.go`, wired in `cmd/ai-env/main.go`).** Five subcommands cover the day-to-day workflow. `ai-env mcp list` prints a column-aligned table of every registered server (name, source, digest, schema hash, scope kinds, policy), sorted by name. `ai-env mcp add <name> --source ... [--digest ...] [--schema-hash ...] [--policy ...] [--scope-filesystem-root workspace_only] [--scope-github-repos current_repo_only] [--scope-github-operations read_only|read_write] [--force]` registers a new server, scaffolding `mcp.yaml` with `version: 1` / `default: deny` if it does not yet exist. `ai-env mcp pin <name> --schema-hash <sha256:hex>` sets or replaces the schema hash on an existing entry; the same hash already pinned is a no-op, and a replacement surfaces the previous value on stderr. `ai-env mcp scan <name> [--source ...] [--digest ...] [--schema-hash ...]` dry-runs `Gateway.AuthorizeLaunch` against operator-supplied candidate metadata and exits non-zero on a block verdict, so it slots into CI. `ai-env mcp remove <name>` deregisters a server; every mutation round-trips through `mcp.ValidateRegistry` before the file is rewritten so a malformed change never hits disk.
- **Audit trail (`mcp-calls.jsonl`).** `internal/run/mcp_calls.go` is the per-run writer: one JSON object per line, fsync after every record, mutex-serialized so concurrent emitters cannot interleave bytes. Each event carries the run ID, an RFC3339 timestamp, the verb (`mcp_authorize_launch` or `mcp_authorize_call`), the outcome (`allow`, `block`, `warn`, or `record`), the server name, the reason string, and the per-call metadata (tool name, scope kind, scope decision). `internal/cli/mcp_logger.go` is the bridge between the gateway and the run-directory writer so a CLI invocation that builds a `Gateway` automatically lands its decisions in the right `mcp-calls.jsonl`. The file is created up front as an empty placeholder by `CreateRunDirectory`, so a run that never invoked MCP still produces a well-formed (empty) JSONL.
- **End-to-end tests (`tests/acceptance/mcp_gateway_test.go`).** The acceptance suite exercises the boundary cases: an unknown server name returns a block, a registered server with a drifted schema hash routes through the per-server policy, a filesystem call whose path escapes the workspace returns a block, and a GitHub call against a non-current repo returns a block. The tests build the gateway against fixture registries and assert both the returned `GatewayOutcome` and the JSON written to `mcp-calls.jsonl`, so the runtime behaviour and the audit trail cannot drift.

The MCP gateway is defense-in-depth: an operator who points an agent at an MCP server URL directly (not through the gateway proxy) opts out of every control above. That opt-out is documented as a residual risk in `docs/residual-risk.md` and called out at the top of `docs/mcp-security.md`.
...
ai-env/
  ...
  internal/run/               # Run IDs, run directory layout, state machine, lifecycle.jsonl,
                              ...
                              # mcp-calls.jsonl writer (plan 09)
  ...
  internal/githubbroker/      # GitHubBroker interface, branch / protected-branch /
                              ...
                              # redaction (plan 07)
  internal/mcp/               # MCP registry loader/validator, schema-hash pinning,
                              # gateway proxy (AuthorizeLaunch / AuthorizeCall),
                              # workspace-only filesystem scope enforcer,
                              # current-repo-only GitHub scope enforcer (plan 09),
                              # BuildRunGateway, per-run mcp-servers.json with
                              # per-server short-lived tokens, JSON-aware
                              # per-direction streaming secret detector (plan 10)
  internal/capability/        # Host capability snapshot (CAP_NET_ADMIN, setns,
                              # userns-remap on Linux; pflog on macOS) (plan 10)
  internal/egress/            # Egress observers and per-run firewall rule lifecycle:
                              # nflog/ (Linux NFLOG via go-nflog), pflog/ (macOS
                              # pflog via tcpdump), rules/ (per-run iptables chain
                              # on Linux, per-run pf rules on macOS) (plan 10)
  internal/policy/            # Policy engine (allow/deny/ask), POSIX shell
                              # tokenizer, HighRiskShellPatterns, shim_install.go
                              # and shim_programs.go canonical shadow set (plans 08, 10)
...
/tmp/ai-env run demo --agent claude --task "say hi"  # supervised run; needs a configured backend + agent CLI
