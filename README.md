...
This repository contains the foundation release: project scaffolding, configuration loading and validation, and the `ai-env new` and `ai-env list` commands. Workspace materialization (Git worktrees, copy-mode), sandbox backends, and the supervisor are tracked in later plans.

## Status

Plan 01 complete. The CLI builds and runs on macOS and Linux, has unit tests for config, scaffold, stack detection, and gitignore handling, and is exercised by a GitHub Actions CI workflow (`gofmt`, `go vet`, `go test -race`, `staticcheck`, `govulncheck`).
...
Scaffolds an `.ai-env/` directory in the current working directory and writes the four configuration files plus a gitignored secrets stub.
...
- `--force`: optional. Overwrite existing configuration files in `.ai-env/`. Without `--force`, the command refuses to clobber any of the generated files.

What it does:

1. Validates the env name.
2. Resolves the source project (current directory or `--from`).
3. Detects whether the source is a Git repository (`.git` present) and records the workspace strategy: `worktree` for Git sources, `copy` for non-Git. Actual workspace materialization lands in Plan 02; Plan 01 only records the strategy.
...
7. If a `.gitignore` exists at the source root, appends any missing required entries under an `# ai-env` section. If no `.gitignore` exists, prints the suggested entries to stdout and lets you add them.
8. Prints a summary with source path, strategy, template, written files, and the `.gitignore` outcome.
...
ai-env/
  ...
  internal/cli/       # Command implementations (RunNew, RunList)
