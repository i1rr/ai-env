// Package podman implements the rootless Podman fallback backend.
//
// This backend is the second reduced-isolation fallback the master plan
// calls out alongside the docker fallback. It speaks to the host's
// `podman` CLI (rootless by default for podman) and brings up a
// container per environment with `--network none` by default. Like the
// docker fallback, it is suitable for development and testing but
// explicitly NOT for high-risk autonomous execution: invoking it for
// autonomous mode requires the operator to opt in via
// --accept-reduced-isolation, and the adapter prints the
// reduced-isolation warning on every Detect that observes acceptance.
//
// Design rules tracked from plan 05 step 10:
//
//  1. Detect with exec.LookPath("podman"). An unavailable binary
...
//     allowlist enforcement, because the rootless podman fallback
...
// in production we always resolve "podman".
const defaultBinary = "podman"

// defaultDetectTimeout caps the `podman version` probe. Detect is on
...
// literal is exported so external callers can reuse it without
// diverging on wording; it matches the docker fallback verbatim.
...
// Options configures the podman fallback adapter. The zero value is
...
// autonomous gate.
type Options struct {
	// Binary is the podman executable to invoke. Empty means "podman".
	Binary string

	// Runner is the function the adapter uses to actually execute the
	// podman CLI. Nil means "use the real os/exec runner". Tests
...
// Runner is the seam tests use to intercept podman invocations. It
...
// non-zero exit code is a clean failure from podman; a non-nil error is
...
// Backend is the podman fallback implementation of backend.Backend.
...
// container in every subsequent podman invocation.
type envState struct {
	spec          backend.EnvSpec
	containerName string
	info          backend.RuntimeInfo
	running       bool
	networkMode   string // "none" or "host"; the value passed to podman run --network
}

// New constructs a podman fallback Backend with sensible defaults. Pass
...
// `podman version`. We only ever extract this single token; everything
// else on the line is treated as opaque log content.
var versionLine = regexp.MustCompile(`v?(\d+\.\d+(?:\.\d+)?)`)

// Detect implements backend.Backend. It checks for the podman binary on
...
func (b *Backend) Detect() backend.BackendStatus {
	...
	if _, err := exec.LookPath(b.binary); err != nil {
		...
		status.Message = fmt.Sprintf("podman not found on PATH (looked for %q): install Podman (rootless recommended) or configure the binary location", b.binary)
		return status
	}
	...
	if err != nil {
		status.Available = false
		status.Message = fmt.Sprintf("podman version probe failed: %v", err)
		return status
	}
	if exitCode != 0 {
		status.Available = false
		status.Message = fmt.Sprintf("podman version exited %d: %s", exitCode, strings.TrimSpace(stderr.String()))
		return status
	}
	...
	if version == "" {
		status.Available = false
		status.Message = "podman version output did not contain a recognizable version string"
		return status
	}
	...
	if b.mode == "autonomous" && !b.accepted {
		...
		status.Message = "podman fallback backend requires --accept-reduced-isolation for autonomous mode (reduced isolation: see WARNING below)\n" + ReducedIsolationWarning
		return status
	}
	...
	// podman release the binary can run is accepted. Operators who
	// want a stricter pin gate the fallback through doctor instead.
	status.VersionSupported = true
	if b.accepted {
		status.Message = "podman fallback backend (reduced isolation; --accept-reduced-isolation acknowledged)"
		b.printWarningOnce()
	} else {
		status.Message = "podman fallback backend (reduced isolation; interactive mode)"
		b.printWarningOnce()
	}
	return status
}

// extractVersion pulls the first semver-ish token from podman's version
...
// Create implements backend.Backend. The rootless podman fallback does
// not have a "create without start" abstraction the way sbx does:
// `podman create` would leave a stopped container behind that Start
// then has to `podman start`, but Start's job in this adapter is to
...
func (b *Backend) Create(spec backend.EnvSpec) (string, error) {
	if spec.Name == "" {
		return "", fmt.Errorf("podman: Create requires a non-empty EnvSpec.Name")
	}
	if spec.WorkspacePath == "" {
		return "", fmt.Errorf("podman: Create requires a non-empty EnvSpec.WorkspacePath")
	}
...
// sanitizeName strips characters podman rejects from container names.
// podman container names match [a-zA-Z0-9][a-zA-Z0-9_.-]*; the adapter
...
func (b *Backend) Start(envID string) (backend.RuntimeInfo, error) {
	...
	b.mu.Unlock()
	if !ok {
		return backend.RuntimeInfo{}, fmt.Errorf("podman: unknown envID %q", envID)
	}
	if env.running {
		return env.info, nil
	}
	...
	// <source>:<target>[:ro]`. Mode is informational on podman (the
	...
	// EnvSpec.UID: podman honors `--user <uid>` for the entrypoint.
	...
	// entrypoint; the agent process is launched via `podman exec`.
	...
	if err != nil {
		return backend.RuntimeInfo{}, fmt.Errorf("podman: podman run spawn failed: %w", err)
	}
	if exitCode != 0 {
		return backend.RuntimeInfo{}, fmt.Errorf("podman: podman run exited %d: %s", exitCode, strings.TrimSpace(stderr.String()))
	}
...
// via `podman exec`. The stream wiring on opts is forwarded directly
...
func (b *Backend) Exec(envID string, cmd backend.Command, opts backend.ExecOptions) (backend.ExecResult, error) {
	...
	b.mu.Unlock()
	if !ok {
		return backend.ExecResult{}, fmt.Errorf("podman: unknown envID %q", envID)
	}
	if cmd.Program == "" {
		return backend.ExecResult{}, fmt.Errorf("podman: Exec requires a non-empty Command.Program")
	}
	if !env.running {
		return backend.ExecResult{}, fmt.Errorf("podman: env %q is not running; call Start first", envID)
	}
	...
	dur := b.now().Sub(start)
	if err != nil {
		return backend.ExecResult{Duration: dur}, fmt.Errorf("podman: podman exec spawn failed: %w", err)
	}
...
// Stop implements backend.Backend. It invokes `podman stop` with the
// requested grace period and signal. A nil signal means "use podman's
// default" (SIGTERM).
func (b *Backend) Stop(envID string, signal os.Signal, timeout time.Duration) error {
	...
	b.mu.Unlock()
	if !ok {
		return fmt.Errorf("podman: unknown envID %q", envID)
	}
	...
	exitCode, err := b.runner(ctx, b.binary, args, nil, &stdout, &stderr, nil, "")
	if err != nil {
		return fmt.Errorf("podman: podman stop spawn failed: %w", err)
	}
	if exitCode != 0 {
		return fmt.Errorf("podman: podman stop exited %d: %s", exitCode, strings.TrimSpace(stderr.String()))
	}
...
// CopyIn implements backend.Backend via `podman cp <src>
// <container>:<dest>`.
func (b *Backend) CopyIn(envID, src, dest string) error {
	...
	if src == "" || dest == "" {
		return fmt.Errorf("podman: CopyIn requires non-empty src and dest")
	}
	...
	exitCode, err := b.runner(ctx, b.binary, args, nil, &stdout, &stderr, nil, "")
	if err != nil {
		return fmt.Errorf("podman: podman cp (in) spawn failed: %w", err)
	}
	if exitCode != 0 {
		return fmt.Errorf("podman: podman cp (in) exited %d: %s", exitCode, strings.TrimSpace(stderr.String()))
	}
	return nil
}

// CopyOut implements backend.Backend via `podman cp <container>:<src>
// <dest>`.
func (b *Backend) CopyOut(envID, src, dest string) error {
	...
	if src == "" || dest == "" {
		return fmt.Errorf("podman: CopyOut requires non-empty src and dest")
	}
	...
	exitCode, err := b.runner(ctx, b.binary, args, nil, &stdout, &stderr, nil, "")
	if err != nil {
		return fmt.Errorf("podman: podman cp (out) spawn failed: %w", err)
	}
	if exitCode != 0 {
		return fmt.Errorf("podman: podman cp (out) exited %d: %s", exitCode, strings.TrimSpace(stderr.String()))
	}
	return nil
}

// ApplyNetworkPolicy implements backend.Backend. The rootless podman
...
func (b *Backend) ApplyNetworkPolicy(envID string, policy backend.NetworkPolicy) error {
	...
	case "host":
		...
		if policy.Default == "deny" && len(policy.AllowDomains) > 0 {
			return fmt.Errorf("podman: cannot enforce allowlist on rootless fallback with --unsafe-host-network; remove allow_domains or run on the docker-sbx backend")
		}
		return nil
	default:
		return fmt.Errorf("podman: env %q has unknown networkMode %q (Start must run first)", envID, env.networkMode)
	}
}

// Stats implements backend.Backend. `podman stats --no-stream` exists,
...
func (b *Backend) Stats(envID string) (backend.ResourceStats, error) {
	...
	b.mu.Unlock()
	if !ok {
		return backend.ResourceStats{}, fmt.Errorf("podman: unknown envID %q", envID)
	}
...
// Destroy implements backend.Backend via `podman rm -f <container>`.
...
func (b *Backend) Destroy(envID string) error {
	...
	if err != nil {
		return fmt.Errorf("podman: podman rm spawn failed: %w", err)
	}
	if exitCode != 0 {
		return fmt.Errorf("podman: podman rm exited %d: %s", exitCode, strings.TrimSpace(stderr.String()))
	}
...
// GatewayAddress implements backend.Backend. The rootless podman
// fallback has no bridge-gateway concept that is meaningful to the
// supervisor: with `--network none` there is no gateway, with
// `--network host` the gateway is the host's outbound default which
// the ProviderProxy picker does not consume. Plan §0.5 documents the
// contract: backends that cannot report a gateway return ("", nil)
// so the supervisor falls through to UnixSocket. An unknown envID
// still returns an error so the supervisor's "env not created" path
// is exercised.
...
// MappedUID implements backend.Backend. Rootless podman runs the
// container's userns inside the operator's host userns; the host-
// side UID the in-sandbox UID maps to is always EnvSpec.UID verbatim
// (or 0 when nil). Plan §0.5: backends that do not implement remap
// return the input UID.
func (b *Backend) MappedUID(envID string) (int, error) {
	b.mu.Lock()
	defer b.mu.Unlock()
	env, ok := b.envs[envID]
	if !ok {
		return 0, fmt.Errorf("podman: unknown envID %q", envID)
	}
	if env.spec.UID != nil {
...
// ProbeImage implements backend.Backend. The rootless podman fallback
// could parse the image's `/etc/passwd`; the plan's iter-4 rationale
// (Plan §0.5) is that probing trusts untrusted image output. The
// fallback returns ("", nil) so the supervisor falls back to the
// policy default `/root` per Plan §0.5.
...
// supervisor uses to install policy on a podman fallback environment.
...
// run reports. It mirrors podman.Name so operators can correlate the
// network entry in run.json with the backend entry: both read "podman".
...
func (a *NetworkPolicyAdapter) Apply(envID string, policy network.NetworkPolicy) error {
	if a.backend == nil {
		return fmt.Errorf("podman: NetworkPolicyAdapter has nil Backend (programmer error)")
	}
	if envID == "" {
		return fmt.Errorf("podman: ApplyNetworkPolicy requires a non-empty envID")
	}
	if err := policy.Validate("interactive"); err != nil {
		return fmt.Errorf("podman: invalid network policy: %w", err)
	}
...
// compile-time checks that the podman fallback satisfies both the
