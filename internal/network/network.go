...
// AlwaysBlockedCIDRs is the set of IP ranges every NetworkPolicy
// instance blocks regardless of operator configuration. The list comes
// from the master plan's "Always blocked by default" section and is
// kept here as the single source of truth so adapters can emit the
// same set when they translate to their backend's native rules.
//
...
// AlwaysBlockedHosts is the set of hostnames every NetworkPolicy
// instance blocks regardless of operator configuration. Like
// AlwaysBlockedCIDRs it lifts the master plan's "Always blocked by
// default" hostnames into a single canonical slice.
var AlwaysBlockedHosts = []string{
	"localhost",
